    ...
    <Modal
      ...
      <ol className="mt-2">
        ...
        <li key={s.name} className="flex gap-3 border-b border-line-soft py-2.5 last:border-0">
            ...
            <span className={`mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full ${TONE_DOT[s.tone]}`} aria-hidden="true" />
...
export function GroupsPage() {
  const { groups, capabilities } = useRegistry();
          ...
          return (
            <section key={g.id}>
              <div className="flex flex-wrap items-baseline gap-3">
                ...
                <span className="ml-auto font-mono text-2xs text-mute">{members.length} capabilities</span>
              </div>
